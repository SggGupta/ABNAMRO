...
class TestTranformation(PySparkTestCase):
    def test_single_space(self):
        ...
        path = './test/top_3_most_sold_products_per_department'

        # Apply the transformation function from before
        top_3_most_sold_products_per_department(df1, df3, path)
        transformed_df = spark.read.csv('./test/top_3_most_sold_products_per_department/*.csv', inferSchema='true')

        expected_df = spark.read.csv('./test/expected_top_3_most_sold_products_per_department/*.csv', inferSchema='true')
